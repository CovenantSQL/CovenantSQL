...
(
    ${NEW_BIN_DIR}/cql rpc -config node_bp_c/config.yaml -bp -name 'MCC.AddTx' -wait-tx-confirm \
        -req '{"Tx": {"TxType": 13, "Miner": "ba0ba731c7a76ccef2c1170f42038f7e228dfb474ef0190dfe35d9a37911ed37", "Enabled": 1}}'
    ${NEW_BIN_DIR}/cql rpc -config node_bp_c/config.yaml -bp -name 'MCC.AddTx' -wait-tx-confirm \
        -req '{"Tx": {"TxType": 13, "Miner": "1a7b0959bbd0d0ec529278a61c0056c277bffe75b2646e1699b46b10a90210be", "Enabled": 1}}'
    ${NEW_BIN_DIR}/cql rpc -config node_bp_c/config.yaml -bp -name 'MCC.AddTx' -wait-tx-confirm \
        -req '{"Tx": {"TxType": 13, "Miner": "9235bc4130a2ed4e6c35ea189dab35198ebb105640bedb97dd5269cc80863b16", "Enabled": 1}}'
) || [[ "${test_case}" == "bp" ]]
