...
func (c *cluster) pollState(s NodeState) ([]*Raft, uint64) {
...
func (c *cluster) GetInState(s NodeState) []*Raft {
...
func makeClusterWithBootstrap(n int, t *testing.T, conf *Config) *cluster {
...
func TestRaft_StartStop(t *testing.T) {
	c := makeClusterWithBootstrap(1, t, nil)
	c.Close()
}
...
func TestRaft_RecoverCluster(t *testing.T) {
	...
	runRecover := func(applies int) {
		...
		c := makeClusterWithBootstrap(3, t, conf)
		defer c.Close()
...
func TestRaft_HasExistingState(t *testing.T) {
	// Make a cluster.
	c := makeClusterWithBootstrap(2, t, nil)
	defer c.Close()
...
func TestRaft_SingleNode(t *testing.T) {
	conf := inmemConfig(t)
	c := makeClusterWithBootstrap(1, t, conf)
	defer c.Close()
	raft := c.rafts[0]
...
func TestRaft_TripleNode(t *testing.T) {
	// Make the cluster
	c := makeClusterWithBootstrap(3, t, nil)
	defer c.Close()
...
func TestRaft_RemoveLeader_NoShutdown(t *testing.T) {
	...
	if isPeer(configuration, leader.localID) {
...
func TestRaft_RemoveFollower_SplitCluster(t *testing.T) {
	// Make a cluster.
	conf := inmemConfig(t)
	c := makeClusterWithBootstrap(4, t, conf)
	defer c.Close()
...
func TestRaft_LeaderLeaseExpire(t *testing.T) {
	// Make a cluster
	conf := inmemConfig(t)
	c := makeClusterWithBootstrap(2, t, conf)
	defer c.Close()
