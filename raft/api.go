...
// configuration on all the Peer servers. There is no need to bootstrap
// Learner and Staging servers.
//
// One sane approach is to bootstrap a single server with a configuration
// listing just itself as a Peer, then invoke AddVoter() on it to add other
...
// the Peer servers. There is no need to bootstrap Learner and Staging
// servers.
...
// NodeState is used to return the current raft state.
...
// SetConfiguration applies new configuration to the FSM
func (r *Raft) SetConfiguration(configuration *Configuration) error {
	// TODO implementation
	return nil
}
